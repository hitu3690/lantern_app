...
RSpec.describe "Microposts", type: :request do
  ...
  def patch_valid_information
    post micropost_path, params: { micropost: { memo: "bbb" } }
  end 
  
  def patch_invalid_information
    post micropost_path, params: { micropost: { memo: nil } }
  end 
  ...
  describe "POST /microposts" do
    ...
    it "succeeds to add a micropost" do
      ...
      expect{ post_valid_information }.not_to change(Micropost, :count).by(1)
      follow_redirect!
      expect(request.fullpath).to eq '/users/1'
    end 
  end 
